...
def upload(request):
    if request.method=='POST':
        proj=Student.objects.get(user=request.user)
        target=os.path.join(settings.REPOS,proj)
        r=Repo(target)
        target=target+request.POST['destn']+'/'
        if request.FILES.has_key('file'):
            ...
            for i in request.FILES.getlist('file'):
                ...
                c=Commit(r.active_branch.name,dialogue%(request.user.username,str(i),time))
                c.save()
...
def getlisting(uname,mode):
    proj=Student.objects.get(user=request.user).project.name
    target=os.path.join(settings.REPOS,proj)
